...
class AutomaticSwiftShieldTests: XCTestCase {
    func testObfuscator() {
        ...
        let obfuscatedFile = AutomaticSwiftShield(basePath: "abc", projectToBuild: "abc", schemeToBuild: "abc", modulesToIgnore: [], protectedClassNameSize: 0, dryRun: true, classesToIgnore: [], excludedPrefixTag: "", excludedSuffixTag: "").generateObfuscatedFile(fromString: originalFile, references: references, obfuscationData: obfuscationData)
    ...
    func testPlistExtractor() {
        let protector = AutomaticSwiftShield(basePath: "abc", projectToBuild: "abc", schemeToBuild: "abc", modulesToIgnore: [], protectedClassNameSize: 0, dryRun: true, classesToIgnore: [], excludedPrefixTag: "", excludedSuffixTag: "")
    ...
    func testPlistPrincipalClassObfuscation() {
        let protector = AutomaticSwiftShield(basePath: "abc", projectToBuild: "abc", schemeToBuild: "abc", modulesToIgnore: [], protectedClassNameSize: 0, dryRun: false, classesToIgnore: [], excludedPrefixTag: "", excludedSuffixTag: "")
        let plist = path(for: "MockPlist", ofType: "plist")
